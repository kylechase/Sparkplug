...
public class Meter extends HashMap<String,TagValue> {
    ...
    public void updateMeter(KuraPayload payload,boolean init){
        ...
        if (c.get(Calendar.MINUTE)==0){
            get(String.format("%s/meter/%s",tagPath,"records/hourly")).updateValue(c.get(Calendar.HOUR_OF_DAY));
            payload.addMetric(String.format("%s/meter/%s",tagPath,"records/hourly"),c.get(Calendar.HOUR_OF_DAY));
        }
    }
